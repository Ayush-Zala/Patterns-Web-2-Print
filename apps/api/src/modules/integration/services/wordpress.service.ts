...
export class WordPressService {
  ...
  async connect(workspaceId: string, integrationId: string, userId: string): Promise<any> {
    const integration = await this.validateWordPressIntegration(workspaceId, integrationId);
  ...
  async rotateSecret(workspaceId: string, integrationId: string, userId: string): Promise<any> {
    ...
    this.auditPublisher.publish({
      action: 'WORDPRESS_ROTATE_SECRET',
      resource: 'Integration',
      resourceId: integrationId,
      userId,
      workspaceId,
  ...
  private async validateWordPressIntegration(workspaceId: string, integrationId: string) {
    ...
    if (integration.type !== WORDPRESS) {
      throw new BadRequestException('Operation only supported for WordPress integrations');
    }
